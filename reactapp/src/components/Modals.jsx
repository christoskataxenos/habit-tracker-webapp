            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <Motion.div
                        ...
                        className="w-full max-w-sm glass-silver p-8 rounded-2xl relative z-10"
                    >
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
                    ...
                    <Motion.div
                        ...
                        exit={{ y: 50, opacity: 0 }}
                        id="add-entry-modal-panel"
                        className="w-full max-w-lg my-auto relative glass-silver p-5 lg:p-8 rounded-2xl shadow-2xl max-h-full sm:max-h-[95vh] overflow-y-visible z-10"
                    >
...
export function DayDetailModal({ isOpen, onClose, dateStr, entries, routines = [], onAddEntry, onEdit, onDelete, onLogRoutine }) {
    ...
    entries.forEach(entry => {
        let startH = 12;
        if (entry.startTime) startH = parseInt(entry.startTime.split(':')[0]);
    ...
    routines.forEach(r => {
        ...
        for (let i = 0; i < duration; i++) {
            const h = (startH + i) % 24;
            if (hourlyData[h].intensity === 0) hourlyData[h].intensity = 20;
        }
    });
            ...
            {isOpen && dateStr && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <Motion.div
                        ...
                        id="day-detail-panel"
                        className="w-full max-w-4xl h-[80vh] bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden flex flex-col md:flex-row relative shadow-[0_0_50px_rgba(0,0,0,0.8)] z-10"
                    >
                        ...
                        <div className="w-full md:w-7/12 bg-[#050505] p-6 lg:p-8 overflow-y-auto custom-scrollbar">
                                            ...
                                            return (
                                                <div key={idx} className={`p-4 rounded-xl border flex items-center justify-between transition-all ${isCompleted ? 'bg-white/5 border-white/5 opacity-50' : 'bg-orange-500/10 border-orange-500/30'}`}>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded bg-black/40 border border-white/10 text-slate-400">
                                                            {isCompleted ? 'COMPLETE' : 'PENDING'}
                                                        </div>
                                                        {!isCompleted && (
                                                            <button
                                                                onClick={() => onLogRoutine(routine)}
                                                                className="p-1.5 rounded-lg bg-orange-500 hover:bg-orange-400 text-black transition-all"
                                                                title="Quick Log Protocol"
                                                            >
                            ...
                            {entries.length === 0 ? (
                                ...
                                <div className="relative space-y-0.5">
                                    ...
                                    {entries.slice().sort((a, b) => (a.startTime || '00:00').localeCompare(b.startTime || '00:00')).map((entry, idx) => (
                                        <div key={idx} className="group relative z-10 pl-16 py-4 hover:bg-white/5 rounded-xl transition-all border border-transparent hover:border-white/5">
                                            <div className="absolute left-2 top-1/2 -translate-y-1/2 w-[50px] text-right">
                                                <div className="text-xs font-mono font-bold text-slate-400 group-hover:text-white transition-colors">{entry.startTime}</div>
                                                <div className="text-[10px] text-slate-600">{entry.endTime}</div>
                                            </div>
                                            <div className={`absolute left-[24px] top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full border border-black ${entry.tag === 'Deep Work' ? 'bg-blue-500' : entry.tag === 'Health' ? 'bg-orange-500' : 'bg-slate-500'} group-hover:scale-150 transition-transform shadow-[0_0_10px_rgba(0,0,0,0.5)]`} />
                                            <div className="flex justify-between items-start gap-4">
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="text-lg text-platinum font-medium leading-none mb-1 group-hover:text-blue-400 transition-colors truncate">{entry.course}</h4>
                                                    <p className="text-sm text-slate-500 font-light line-clamp-1 group-hover:line-clamp-none transition-all">{entry.topic || 'No details provided.'}</p>
                                                </div>
                                                <div className="flex flex-col items-end gap-1 shrink-0">
                                                    <div className="flex items-center gap-2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            onClick={() => onEdit(entry)}
                                                            className="p-1 rounded bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 transition-all border border-blue-500/20"
                                                            title="Edit Entry"
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-1 rounded bg-red-500/10 hover:bg-red-500/20 text-red-400 transition-all border border-red-500/20"
                                                            title="Delete Entry"
                                                        >
                                                    ...
                                                    <div className="flex flex-col items-end gap-1">
                                                        <span className="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-white/5 text-slate-400 border border-white/5 whitespace-nowrap">{entry.tag || 'GENERAL'}</span>
                                                        <span className="text-[10px] font-mono text-slate-600 font-bold">{entry.hours}h</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <Motion.div
                        ...
                        className="w-full max-w-md glass-silver p-8 rounded-2xl relative z-10"
                    >
