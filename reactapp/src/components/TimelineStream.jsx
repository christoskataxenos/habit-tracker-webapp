...
const TimelineStream = ({ entries, onDelete, onEdit }) => {
    ...
    return (
        <div className="relative pl-4 h-full overflow-y-auto no-scrollbar pt-2 pb-12 overflow-x-hidden">
                    ...
                    <motion.div
                        ...
                        onDragEnd={(e, info) => {
                            if (info.offset.x < -100) {
                                vibrate(HapticPatterns.medium); // Feedback
                                onDelete(entry.id);
                            }
                        }}
                        className="group relative mb-3 last:mb-0 pl-6 cursor-grab active:cursor-grabbing touch-pan-y"
                    >
                        ...
                        <div className="card-metallic p-3 flex flex-col group-hover:translate-x-1 transition-transform relative">
                            ...
                            {/* Buttons still exist for desktop users */}
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                <button onPointerDown={(e) => { e.stopPropagation(); }} onClick={() => onEdit(entry)} className="text-blue-400 hover:text-blue-300 transition-all p-1.5 hover:bg-black/40 rounded shadow-sm border border-transparent hover:border-white/10" title="Edit Entry" aria-label="Edit Entry">
                                    <Edit3 className="w-3 h-3" />
                                </button>
                                <button onPointerDown={(e) => { e.stopPropagation(); }} onClick={() => onDelete(entry.id)} className="text-red-400 hover:text-red-300 transition-all p-1.5 hover:bg-black/40 rounded shadow-sm border border-transparent hover:border-white/10" title="Delete Entry" aria-label="Delete Entry">
